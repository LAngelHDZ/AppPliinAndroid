...
import com.example.pliin.apppliin.domain.usecase.manifest.AddGuideManifestUseCase
import com.example.pliin.apppliin.domain.usecase.manifest.CreateManifestUseCase
import com.example.pliin.apppliin.domain.usecase.user.GetAllEmployeesUseCase
import com.example.pliin.apppliin.domain.usecase.manifest.GetConsecManUseCase
...
import com.example.pliin.apppliin.domain.usecase.manifest.RegisterDatosPqtUseCase
import com.example.pliin.apppliin.domain.usecase.manifest.RegisterDireccionUseCase
import com.example.pliin.apppliin.domain.usecase.manifest.UpdateManifestUseCase
import com.example.pliin.apppliin.domain.usecase.manifest.ValidateExistsAddressUseCase
import com.example.pliin.apppliin.domain.usecase.manifest.ValidateExistsDataPqtUseCase
import com.example.pliin.apppliin.domain.usecase.guides.ValidateGuideSystemUseCase
...
@HiltViewModel
class VMAViewModel @Inject constructor(
...
//=======================================================
    ...
    private val _listGuide = MutableLiveData<List<DataItem?>?>()
    var listGuide: LiveData<List<DataItem?>?> = _listGuide
...
//De inicio consulkta los datos del manifiesto cuando se seleciona, consulta las guias y su informacion
    fun loadData(
        nameEmploye:String,
        idRecord:String,
        route:String,
...
//    REsetea las variables usadas en el formulario de datos de direccion y de los paquetes
    fun clearForm() {
        _nombre.value = ""
        _telefono.value = ""
        _dir1.value = ""
...
//    Resetea las variables a un estado inical o vacio
    fun reset() {
        btnpushCrate=0
        _nombre.value = ""
        _telefono.value = ""
...
//    Activa un modal con un mensaje para que el usaurio indique si desea continuar con el proceso de lo contrario cancelarlo
    fun onDialogaddPlaneation() {
        _isDialogLoadEnable.value = true
        _messageGuideValidate.value = "¿Quiere agregar este manifiesto a su planeaciòn?"
    }
...
//    Ejecuta la accion de continujar al siguiente formuklario para llenar los datos de la direccion de la guia y los datos del paquete
...
//    Agrega los datos de la direccion y datos del paquete en una lista
...
//    Scannea la guia y realiza las valiaciones para que no se repita y sea una guia de formato correcto
...
//    Quita de la lista local la guia seleccionada
...
//    GEnera la clave del premanifiesto
...
//    Descaga la informacion de manifiesto y lo guarda en la base de datos local, esto pasa cuando el operador elige el manifiesto para trabajarlo
    fun downloadManifest(navigationController: NavHostController){
        viewModelScope.launch{

            if (btncountAplication <= 0){
                _isLoadingDataGuide.value = true
                val dataManifest= getOneManifestUseCase.invoke(clavePreManifest.value!!)?.get(0)?.fieldData!!
                val responseSManifest = saveManifestDBUseCase.invoke(dataManifest)
                val nameEmploye = nameEmployye.value
                ...
                val updateManifest =updateManifestUseCase.invoke(dataDto)
//                val responseSGuides = getGuidesManifestUseCase.invoke(folioManifest.value!!)
                val ok = saveGuidesManDBUseCase.invoke(listGuide.value)
//                if (updateManifest){
//                    _isLoadingDataGuide.value = false
//                }
...
//    Obtiene el numero consecutivo del ultimo manifiesto creado y lo incremeneta para asignarlo al nuevo manifiesto a crear
...
//    Seleccciona el nombre del empleado,que esta logueado en la aplicación
...
//    Actualiza el manifiesto cambiando el status, elk numero de guias asignadas, total de paquetes y el nombre del operador
    fun updateManifest() {
        viewModelScope.launch {
...
//    Carga las guias agregadas al manifiesto al sistema
    fun loadGuideServer() {
        var progres = 0
